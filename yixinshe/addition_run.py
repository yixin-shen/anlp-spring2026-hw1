...
def train_one_epoch(model, loader, optimizer, device):
    ...
    for inputs, targets in loader:
        ...
        # Locate "=" position in each sequence
        equals_indicator = (targets == EQUALS_TOKEN).long()
        cumulative_equals = torch.cumsum(equals_indicator, dim=1)
        answer_mask = (cumulative_equals > 0) & (targets != EQUALS_TOKEN)

        flat_predictions = predictions.view(batch_size * seq_len, vocab_size)
        flat_targets = targets.view(batch_size * seq_len)
        flat_mask = answer_mask.view(batch_size * seq_len)

        valid_indices = flat_mask & (flat_targets >= 0) & (flat_targets < vocab_size)

        if valid_indices.any():
            loss = F.cross_entropy(flat_predictions[valid_indices], flat_targets[valid_indices])
        else:
            loss = flat_predictions.sum() * 0.0
...
def evaluate_loss(model, loader, device):
    ...
    for inputs, targets in loader:
        inputs, targets = inputs.to(device), targets.to(device)
        predictions, _ = model(inputs, targets=targets)
        ...
        flat_predictions = predictions.view(batch_size * seq_len, vocab_size)
        flat_targets = targets.view(batch_size * seq_len)
        flat_answer_mask = answer_mask.view(batch_size * seq_len)

        valid_answer_tokens = flat_answer_mask & (flat_targets >= 0) & (flat_targets < vocab_size)

        if valid_answer_tokens.any():
            loss = F.cross_entropy(flat_predictions[valid_answer_tokens], flat_targets[valid_answer_tokens])
        else:
            loss = flat_predictions.sum() * 0.0
...
def load_model(save_dir, filename, model, device):
    ...
    # Allow numpy arrays in checkpoint for PyTorch 2.6+
    try:
        import numpy as np
